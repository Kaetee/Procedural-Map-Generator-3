...
namespace MapMaker {
	// Taken and slightly adapted from https://lotsacode.wordpress.com/2010/02/24/perlin-noise-in-c/
	public class Perlin {
		...
		public double Noise(double x, double y, double z, int octaves, double persistance) {
			double totalStrength = 0.0;
			double currentMultiplier = 2.0;
			double output = 0.0;

			for (int i = 0; i < octaves; i++) {
				currentMultiplier *= persistance;
				totalStrength += currentMultiplier;
				output += Noise(x * (i + 1), y * (i + 1), z * (i + 1)) * currentMultiplier;
			}

			output /= totalStrength;
